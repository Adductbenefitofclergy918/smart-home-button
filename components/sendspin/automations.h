...
template<typename... Ts> class SendCommandAction : public Action<Ts...>, public Parented<SendspinHub> {
 public:
  TEMPLATABLE_VALUE(SendspinControllerCommand, command);

  void play(const Ts &...x) override { this->parent_->send_client_command(this->command_.value(x...)); }
};

template<typename... Ts> class SendVolumeCommandAction : public Action<Ts...>, public Parented<SendspinHub> {
 public:
  ...
  void play(const Ts &...x) override {
    ...
    this->parent_->send_client_command(SendspinControllerCommand::VOLUME, volume, std::nullopt);
  }
};
