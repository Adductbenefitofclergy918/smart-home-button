...
void SendspinTextSensor::setup() {
  switch (this->metadata_type_) {
    ...
    case SendspinMetadataTypes::PLAYBACK_STATE: {
      this->parent_->add_group_update_callback([this](const GroupUpdateObject &group) {
        if (group.playback_state.has_value()) {
          switch (group.playback_state.value()) {
            case SendspinPlaybackState::PLAYING:
              this->publish_if_changed_("playing");
              break;
            case SendspinPlaybackState::STOPPED:
            default:
              this->publish_if_changed_("stopped");
              break;
          }
        }
      });
