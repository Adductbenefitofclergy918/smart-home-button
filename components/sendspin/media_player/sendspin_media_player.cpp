...
void SendspinMediaPlayer::setup() {
  ...
  this->parent_->add_sensor_callback([this](const SendspinSensorUpdate &sensor_update) {
    if (sensor_update.type != SendspinSensorTypes::CONTROLLER_VOLUME) {
      return;
    }
    float new_volume = sensor_update.value / 100.0f;
    if (new_volume < 0.0f) {
      new_volume = 0.0f;
    } else if (new_volume > 1.0f) {
      new_volume = 1.0f;
    }
    if (std::fabs(this->volume - new_volume) > 0.005f) {
  ...
  this->volume = this->parent_->get_controller_state().volume / 100.0f;
  this->publish_state();
}
...
media_player::MediaPlayerTraits SendspinMediaPlayer::get_traits() {
  ...
  traits.add_feature_flags(media_player::MediaPlayerEntityFeature::PLAY | media_player::MediaPlayerEntityFeature::PAUSE |
  ...
  if (command.has_value()) {
    switch (command.value()) {
      ...
      case media_player::MEDIA_PLAYER_COMMAND_VOLUME_UP:
        this->volume = std::min(100, this->parent_->get_controller_state().volume + this->volume_increment_) / 100.0f;
        this->publish_state();
        this->parent_->send_client_command(SendspinControllerCommand::VOLUME, static_cast<uint8_t>(this->volume * 100.0f),
                                           std::nullopt);
        break;
