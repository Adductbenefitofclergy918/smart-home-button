...
static bool process_artwork_channel_object(const JsonObject channel_object, ServerArtworkChannelObject *channel,
  ...
  if (require_all_fields) {
    bool has_width = channel_object["width"].is<JsonVariant>() || channel_object["media_width"].is<JsonVariant>();
    bool has_height = channel_object["height"].is<JsonVariant>() || channel_object["media_height"].is<JsonVariant>();
    if (!channel_object["source"].is<JsonVariant>() || !channel_object["format"].is<JsonVariant>() || !has_width ||
        !has_height) {
  ...
  } else if (channel_object["media_width"].is<JsonVariant>()) {
    channel->width = channel_object["media_width"].as<uint16_t>();
  }

  if (channel_object["height"].is<JsonVariant>()) {
    channel->height = channel_object["height"].as<uint16_t>();
  } else if (channel_object["media_height"].is<JsonVariant>()) {
    channel->height = channel_object["media_height"].as<uint16_t>();
  }
  ...
  return json::build_json([msg](JsonObject root) {
    ...
    if (msg->artwork_v1_support.has_value()) {
      ...
      for (const auto &channel : msg->artwork_v1_support.value().channels) {
        ...
        channel_obj["width"] = channel.media_width;
        channel_obj["height"] = channel.media_height;
  ...
  return json::build_json([msg](JsonObject root) {
    ...
    if (msg->artwork.has_value()) {
      ...
      if (artwork.media_width.has_value()) {
        root["payload"]["artwork"]["width"] = artwork.media_width.value();
        root["payload"]["artwork"]["media_width"] = artwork.media_width.value();
      }
      if (artwork.media_height.has_value()) {
        root["payload"]["artwork"]["height"] = artwork.media_height.value();
